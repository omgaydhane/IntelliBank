...
class CLNNModelManager:
    ...
    def evaluation(self, args, data, save_results=True, plot_cm=True, input_text=None):
        """final clustering evaluation on test set"""
        # get features
        feats_test, labels, embeddings = self.get_features_labels(data.test_dataloader, self.model, args)
        ...
        if input_text:
            self.evaluate_user_input(input_text, km, self.tokenizer, args)
            exit(0)
        ...
        model_save_path = '/content/drive/MyDrive/MTP_CLNN/km.joblib'
        dump(km, model_save_path)

        # save results
        if save_results:
            self.save_results(args)
        
        return feats_test, labels, embeddings

    def evaluate_user_input(self, input_text, km, tokenizer, args):
        #model.eval()
        ...
        segment_ids = torch.zeros(input_ids.shape, dtype=torch.long).to(args.device)
        
        with torch.no_grad():
            feature = self.model({"input_ids":input_ids, "attention_mask": attention_mask, "token_type_ids": segment_ids}, output_hidden_states=True)["hidden_states"]
            km = load('/content/drive/MyDrive/MTP_CLNN/km.joblib')
            predicted_cluster = km.predict(feature.cpu().numpy())
            predicted_label = data.all_label_list[predicted_cluster[0]]
    ...
    def get_features_labels(self, dataloader, model, args):
        ...
        # Assuming total_embeddings is what you need, adjust the dimension as per your model
        total_embeddings = torch.empty((0, 768)).to(self.device)
        ...
        for batch in tqdm(dataloader, desc="Extracting representation"):
            ...
            with torch.no_grad():
                ...
                # Adjust this line based on your model's output for embeddings
                embeddings = feature[-1]  # Example, using the last hidden state as embedding
        ...
        return total_features, total_labels, total_embeddings
    ...
    def get_args():
        parser = argparse.ArgumentParser(description='CLNN Evaluation')
        parser.add_argument('--input_text', type=str, default='', help='User input text for evaluation')
...
if __name__ == '__main__':
    ...
    feats_test, labels, embeddings = manager.evaluation(args, data)
    results = manager.save_results(args)
    #plot_embeddings(embeddings, labels, data.num_labels)
    ...
    def save_model_to_pkl(model, file_path):
      ...
      with open(file_path, 'wb') as f:
        pickle.dump(model_to_save.state_dict(), f)

    model_path = "/content/drive/MyDrive/MTP_CLNN/model.pkl"
    save_model_to_pkl(manager.model, model_path)
    ...
    # Call the evaluation function with the user input
    print('Evaluation with User Input begin...')
    if input_text:
        manager.evaluation(args, data, input_text=input_text)
    print('Evaluation with User Input finished!')
